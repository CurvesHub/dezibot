...
void MultiColorLight::begin(void){
    rgbLeds.begin();
    this->turnOffLed();
    Log::d(INFOLOG, RGB_LIGHT_COMP, "begin multi color light");
};
...
void MultiColorLight::blink(uint16_t amount,uint32_t color, leds leds, uint32_t interval){
    Log::d(INFOLOG, RGB_LIGHT_COMP, String(leds) + " blinking " + String(amount) + " times with interval " + String(interval) + " ticks", String(color));
